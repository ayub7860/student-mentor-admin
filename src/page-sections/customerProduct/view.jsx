...
export default function Add (props) {
  ...
  React.useEffect(()=> {
    setTableData(props.selectedViewRecord.tableData)
  },[props.selectedViewRecord.tableData])

 
  return (
    <Fragment>
      <Dialog className='z-40' handler={closeDialog} open={props.isViewOpen} size={isMobile ? 'xxl' : 'xxl'}>
        <DialogHeader className='bg-gray-100 text-center justify-center'>Customer Product Tracking </DialogHeader>
        <DialogBody divider>
          ...
          <table className="min-w-full bg-white border border-gray-200">
            <thead className='text-left bg-gray-300'>
              <tr>
                ...
                {/* <th className="py-2 px-2 border-b">Staff Name</th>
            ...
            <tbody>
              {tableData && tableData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-100 text-left text-sm">
                  ...
                  <td className="py-1 px-2 border-b">{item.staffName || item.branchName || item.userName}</td>
                  {/* <td className="py-1 px-2 border-b">{item.branchName}</td> */}
