...
const Add = React.lazy(() => import('../page-sections/countryMaster/add'))
const Edit = React.lazy(() => import('../page-sections/countryMaster/edit'))

export default function CountryMasterHolder () {
  ...
  React.useEffect(() => {
    document.title = 'Aditya-Anangha | Country Master'
  ...
  const getTableRecordByPage = (currentPage, perPage, orderBy, orderDirection, searchValue) => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/countryApi/getTableCountry`, {
        currentPage,
        perPage,
        orderBy,
        orderDirection,
  ...
  const changeStatus = (id, value) => {
    const url = `${import.meta.env.VITE_API_URL}/api/countryApi/changeStatusCountry`
  ...
  const prepareForDownload = () => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/countryApi/getCSVTableAdminCountry`)
      .then(async (response) => {
        if (response.status === 200) {
  ...
  const processCSVData = (tempData) => {
    ...
    csvDataTemp.push(['Sr.No.', 'Country Name', 'Status', 'Created At', 'Updated At'])
          ...
          <div className='flex flex-col md:flex-row justify-between'>
            ...
            <div className='flex flex-col md:flex-row gap-2'>
              <div className='bg-white dark:bg-gradient-to-br from-blue-gray-700 to-blue-gray-800 rounded-md border-0'>
                <Input className='border-0 focus:border-0 text-black dark:text-white' enterKeyHint='search'
                        ...
                        ? (
                            <div className='flex flex-row gap-2'>
                                <button className='px-2 py-1 text-white rounded shadow-blue-gray-500 justify-center'>
                                    <CSVLink data={csvData} filename={'country-master.csv'}><i className='fas fa-download' /></CSVLink>
                                  </button>
                              </div>
                          )
                        : (
              ...
              <thead>
                <tr>
                  ...
                  <TableHeaderCell key='name' columnName='name' handleOrderBy={handleOrderBy}
                                                 isOrderByAvailable={true} orderBy={tableProp.orderBy}
                                                 orderDirection={tableProp.orderDirection} text='Country_Name'
                  />
