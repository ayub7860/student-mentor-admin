...
const Add = React.lazy(() => import('../page-sections/staffMaster/add'))
const Edit = React.lazy(() => import('../page-sections/staffMaster/edit'))
const StaffShift = React.lazy(() => import('../page-sections/staffMaster/shiftStaff'))
const BranchShift = React.lazy(() => import('../page-sections/staffMaster/branchShift'))
const View = React.lazy(() => import('../page-sections/staffMaster/importExcel'))
...
export default function CustomerMasterHolder () {
  ...
  const [selectedBranchRecord, setSelectedBranchRecord] = useState({})
  ...
  const getTableRecordByPage = (currentPage, perPage, orderBy, orderDirection, searchValue) => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/adminStaffApi/getTableStaff`, {
        currentPage,
        perPage,
        orderBy,
        orderDirection,
  ...
  const blockStaff = (data) => {
    setSelectedRecord(data);
    setIsAddOpen(true)
  }

  const shiftToBranch = (data) => {
    setSelectedBranchRecord(data);
    setIsBranchOpen(true)
  }

  const deleteStaff = (data) => {
    if(window.confirm('Are you sure want to in-active')){
      axios
      .post( `${import.meta.env.VITE_API_URL}/api/adminStaffApi/deleteStaff`,{
          id: data.id,
      })
      .then((response) => {
          if (response.status === 200) {
  ...
  const prepareForDownload = () => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/adminStaffApi/getCSVTableAdminStaff`)
      .then(async (response) => {
        if (response.status === 200) {
  ...
  return (
    <div className='mt-12 mb-8 flex flex-col gap-12 animate-fade-in transform'>
      <Card className='bg-white dark:bg-gradient-to-br from-blue-gray-700 to-blue-gray-800'>
            ...
            <div className='flex flex-col md:flex-row gap-2'>
              ...
              <div className='flex flex-row gap-2'>
                <>
                  ...
                  <div className="flex flex-row gap-2">
                    <Tooltip content="Import Excel" className="text-xs p-1">
                        <Button
                            onClick={event => {
                                event.preventDefault();
                                importExcel()
                            }}
                            className="inline-flex self-center items-center gap-2" variant="outlined" color="white"
                            size="sm">
                        ...
                        ? (
                            <div className='flex flex-row gap-2'>
                                <button className='px-2 py-1 text-white rounded shadow-blue-gray-500 justify-center'>
                                    <CSVLink data={csvData} filename={'staff-master.csv'}><i className='fas fa-download' /></CSVLink>
                                  </button>
                              </div>
                          )
                        : (
        ...
        <CardBody className='overflow-x-auto px-0 pt-0 pb-2 bg-white dark:bg-gradient-to-br from-blue-gray-700 to-blue-gray-800 text-blue-gray-600 dark:text-gray-200'>
          <div
            ...
            >
            <div style={{ width: '180%' }}></div> {/* Adjust width to match your table */}
          </div>
            ...
            <table className='w-full min-w-[640px] table-auto'>
              <thead>
                <tr>
                  ...
                  />
                  <TableHeaderCell key='password' columnName='password' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Password'
                  />
                  ...
                  />
                  <TableHeaderCell key='otherNumber' columnName='otherNumber' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Another_Mobile'
                  />                 
                  <TableHeaderCell key='lastYearTarget' columnName='lastYearTarget' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Achieved Target'
                  />
                   <TableHeaderCell key='target' columnName='target' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Fresh Target'
                  />
                  ...
                  />
                  <TableHeaderCell key='staffCode' columnName='staffCode' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Staff_Code'
                  />   
                  <TableHeaderCell key='division' columnName='division' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Division'
                  />
                  ...
                  />   
                  <TableHeaderCell key='leavingDate' columnName='leavingDate' handleOrderBy={handleOrderBy}
                      isOrderByAvailable={true} orderBy={tableProp.orderBy}
                      orderDirection={tableProp.orderDirection} text='Leaving_Date'
                  />                 
                    ...
                    return (
                      <tr key={rowObj.id}>
                          <td className='py-2 px-2 border-b border-blue-gray-50  items-center'>
                              <div className='flex flex-row gap-3'>
                                  ...
                                  <>
                                    ...
                                    {rowObj.status === 1 &&
                                      <Tooltip className='text-xs p-1' content='branch shift'>
                                        <Typography
                                          as='button'
                                          className='text-base font-semibold text-green-600'
                                          onClick={event => { event.preventDefault(); shiftToBranch(rowObj) }}
                                        >
                                    ...
                                    {rowObj.status === 1 &&
                                      <Tooltip className='text-xs p-1' content='Staff In-Active'>
                                        <Typography
                                          ...
                                          onClick={event => { event.preventDefault(); deleteStaff(rowObj) }}
                                        >
                                      ...
                                      {/* {rowObj.status === 1 &&
                                        <Tooltip className='text-xs p-1' content='block'>
                                          <Typography
                                            as='button'
                                            className='text-base font-semibold text-red-600'
                                            onClick={event => { event.preventDefault(); blockStaff(rowObj) }}
                                          >
                          ...
                          <td className='py-1 px-2 border-b border-blue-gray-50'>
                            {rowObj.name && (
                              <Typography className='text-xs font-semibold text-blue-gray-600 dark:text-gray-200 w-60'>
                                {rowObj.name}
                              </Typography>
                            )}
                          </td> 
                          <TableCell text={rowObj.key} />
                          <TableCell text={rowObj.mobile} /> 
                          <TableCell text={rowObj.otherNumber} />    
                          <TableCell text={rowObj.lastYearTarget} />     
                          <TableCell text={rowObj.target} />                                           
                          {/* <TableCell text={rowObj.address} />  */}
                          <td className='py-1 px-2 border-b border-blue-gray-50'>
                            {rowObj.address && (
                              <Typography className='text-xs font-semibold text-blue-gray-600 dark:text-gray-200 w-96'>
                                {rowObj.address}
                              </Typography>
                            )}
                          </td> 
                          ...
                          <TableCell text={rowObj.staffCode} />
                          <TableCell text={rowObj.division} />      
                          <TableCell text={rowObj.joiningDate} />
                          <TableCell text={rowObj.leavingDate} />                       
                          <td className='py-1 px-2 border-b border-blue-gray-50'>
                              <Chip
      ...
      <Suspense fallback={<div />}>
        ...
        <BranchShift isBranchOpen={isBranchOpen} refreshTableData={refreshTableData} selectedBranchRecord={selectedBranchRecord} setIsBranchOpen={setIsBranchOpen} />
        <View isImportExcelOpen={isImportExcelOpen} setIsImportExcelOpen={setIsImportExcelOpen}
