...
import { fetchData } from '@/hooks/fetchData'
  ...
  export default function CustomerProductMasterHolder () {
    ...
    const getTableRecordByPage = (currentPage, perPage, orderBy, orderDirection, searchValue, fromDate, toDate) => {
      axios
        .post(`${import.meta.env.VITE_API_URL}/api/adminCustomerProductApi/getTableCustomerProduct`, {
          currentPage,
          perPage,
          orderBy,
          orderDirection,
        ...
        <Card className='bg-white dark:bg-gradient-to-br from-blue-gray-700 to-blue-gray-800'>
          ...
          <CardBody className='overflow-x-auto px-0 pt-0 pb-2 bg-white dark:bg-gradient-to-br from-blue-gray-700 to-blue-gray-800 text-blue-gray-600 dark:text-gray-200'>
            ...
            <div
              ...
              >
              <div style={{ width: '170%' }}></div> {/* Adjust width to match your table */}
            </div>
              ...
              <table className='w-full min-w-[640px] table-auto'>
                <thead>
                  <tr>
                    ...
                    />
                    <TableHeaderCell key='accNumber' columnName='accNumber' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                    ...
                    />
                    <TableHeaderCell key='renewalAmount' columnName='renewalAmount' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                        orderDirection={tableProp.orderDirection} text='Renewal_Amount'
                    />
                    <TableHeaderCell key='days' columnName='days' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                        orderDirection={tableProp.orderDirection} text='Days'
                    />
                            ...
                            <td className='py-2 px-2 border-b border-blue-gray-50  items-center'>
                                <div className='flex flex-row gap-3'>
                                    ...
                                    <Tooltip content="edit" className="text-xs p-1">
                                      <Link to={`/admin/edit-customer-product-master/${rowObj.id}`}>
                                        <i className="fas fa-pen-to-square text-base font-semibold text-blue-600"></i>
                                      </Link>
                                    </Tooltip>
                                    {rowObj.exitStatus === 0 &&
                                      <>
                                        {(rowObj.status === 1 || rowObj.status === 2 ) && rowObj.comissionStatus === 1 &&
                                        ...
                                        {/* {rowObj.status === 1 && 
                                          <Tooltip className='text-xs p-1' content='Account close'>
                                            <Typography
                                              as='button'
                                              className='text-base font-semibold text-red-600'
                                              onClick={event => { event.preventDefault(); accountOpenAndCloseByAdmin(rowObj, 3) }}
                                            >
                                        ...
                                        {/* {rowObj.status === 3 && 
                                          <Tooltip className='text-xs p-1' content='Account open'>
                                            <Typography
                                              as='button'
                                              className='text-base font-semibold text-green-600'
                                              onClick={event => { event.preventDefault(); accountOpenAndCloseByAdmin(rowObj, 1) }}
                                            >
                                              <i class="fa-solid fa-user-slash"></i>
                                            </Typography>
                                          </Tooltip>
                                        } */}
                                        {/* {rowObj.status === 1 && rowObj.comissionStatus === 1 &&
                                        <Tooltip className='text-xs p-1' content='assign comission'>
                                          <Typography
                                            as='button'
                                            className='text-base font-semibold text-blue-600'
                                            onClick={event => { event.preventDefault(); addComission(rowObj) }}
                                          >
                                            <i className="fa-solid fa-indian-rupee-sign"></i>
                                          </Typography>
                                        </Tooltip>
                                        }                                      
                                        {rowObj.status === 1 && rowObj.comissionStatus === 2 &&
                                          <Tooltip className='text-xs p-1' content='reset comission'>
                                            <Typography
                                              as='button'
                                              className='text-base font-semibold text-red-600'
                                              onClick={event => { event.preventDefault(); resetComission(rowObj) }}
                                            >
                                              <i className="fa-solid fa-indian-rupee-sign"></i>
                                            </Typography>
                                          </Tooltip>
                                    ...
                                    {rowObj.exitStatus === 1 &&
                                      <>
                                        ...
                                        <p className='flex items-center text-red-500 text-xs font-bold min-w-full mr-2'>EX-STAFF</p>
                                      </>
                                    }
                                  </div>
                              </td>
