import {
  ...
  const Add = React.lazy(() => import('../page-sections/product/add'))
  const Edit = React.lazy(() => import('../page-sections/product/edit'))
  ...
  export default function CustomerMasterHolder () {
    ...
    React.useEffect(() => {
      document.title = 'Aditya-Anangha | Product Master'
    ...
    const getTableRecordByPage = (currentPage, perPage, orderBy, orderDirection, searchValue, fromDate, toDate) => {
      axios
        .post(`${import.meta.env.VITE_API_URL}/api/adminProductApi/getTableProduct`, {
          currentPage,
          perPage,
          orderBy,
          orderDirection,
    ...
    const prepareForDownload = () => {
      axios
        .post(`${import.meta.env.VITE_API_URL}/api/adminProductApi/getCSVTableAdminProduct`)
        .then(async (response) => {
          if (response.status === 200) {
    ...
    const processCSVData = (tempData) => {
      ...
      for (const obj of tempData) {
        ...
        const tempProductType = getProductTypeLabel(productType)
    ...
    const getTableData = () => {
      if(formData.fromDate && formData.toDate){
        getTableRecordByPage(1, 50, 'createdAt', 'desc', '', formData.fromDate, formData.toDate)
                          ...
                          ? (
                              <div className='flex flex-row gap-2'>
                                  <button className='px-2 py-1 text-white rounded shadow-blue-gray-500 justify-center'>
                                      <CSVLink data={csvData} filename={'product-master.csv'}><i className='fas fa-download' /></CSVLink>
                                    </button>
                                </div>
                            )
                          : (
                ...
                <thead>
                  <tr>
                    ...
                    <TableHeaderCell key='glCode' columnName='glCode' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                        orderDirection={tableProp.orderDirection} text='GlCode'
                    />
                    ...
                    />    
                     <TableHeaderCell key='interestRate' columnName='interestRate' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                        orderDirection={tableProp.orderDirection} text='Interest_Rate'
                    />    
                    ...
                    />
                    <TableHeaderCell key='totalProductAmount' columnName='totalProductAmount' handleOrderBy={handleOrderBy}
                        isOrderByAvailable={true} orderBy={tableProp.orderBy}
                        orderDirection={tableProp.orderDirection} text='Total_Amount'
                    />
                    ...
                    (rowObj, key) => {
                      return (
                        <tr key={rowObj.id}>
                            ...
                            <TableCell text={rowObj.interestRate} />  
                            <TableCell text={rowObj.comissionAmount} />  
                            <TableCell text={rowObj.comissionPercentage} />  
                            <TableCell text={rowObj.totalCustomerProduct} />
                            <TableCell text={rowObj.totalActiveCustomerProduct} />
                            <TableCell text={rowObj.totalProductAmount ? rowObj.totalProductAmount : '0'} />
