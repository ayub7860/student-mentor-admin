...
export default function EditCustomerProductMasterHolder (props) {
  ...
  const closeDialog = () => {
    navigate('/admin/expired-customer-product-master', { replace: true })
  }
  ...
  const submitData = async () => {
    ...
    if (!hasError) {
      ...
      try {
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/adminCustomerProductApi/updateExpiredCustomerProduct`, data)
      ...
      <Card className='animate-fade-in transform'>
        ...
        <CardBody className='px-1 md:px-2 lg:px-4 pt-1 pb-2'>
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-3 w-full'>
            ...
            <Input type='number' required label='Fresh Amount' name='accAmount' value={accAmount} onChange={handleTextChange} /> 
            <Input type='number' label='Renewal Amount' name='renewalAmount' value={renewalAmount} onChange={handleTextChange} /> 
